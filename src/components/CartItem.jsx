...
const CartItem = ({
  ...
  const { isDarkMode, handleCartAmountChange, cartProducts, removeCartItem } =
    useGlobalContext();
    ...
    <>
      <section className="p-4 flex flex-col sm:items-center gap-3 sm:grid sm:grid-cols-4 ">
        ...
        <div className="amount flex flex-col gap-2">
          <>
            ...
            <select
              ...
              onChange={(e) => {
                ...
                handleCartAmountChange(id, parseInt(e.target.value));
              }}
              id={"name"}
            >
              {options.map((option) => {
          ...
          <button className="remove w-[20%]" onClick={() => removeCartItem(id)}>
            remove
          </button>
        </div>
